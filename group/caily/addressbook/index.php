			...
			<div class="d-flex row">
				...
				<div class="col-md-6">
					...
					if ($view->permitted($hash['category'], 'add')) {
						echo '<li><a class="btn btn-danger" href="add.php'.$view->parameter(array('folder'=>$_GET['folder'])).'">アドレス追加</a></li>';
					}
					if (count($hash['list']) <= 0) {
						$attribute = ' onclick="alert(\'出力するデータがありません。\');return false;"';
					}
					?>
						<li><a class="btn btn-primary" href="csv.php<?=$view->parameter(array('sort'=>$_GET['sort'], 'desc'=>$_GET['desc'], 'search'=>$_GET['search'], 'folder'=>$_GET['folder'], 'type'=>0))?>"<?=$attribute?>>CSV出力</a></li>
					</ul>
				</div>
			</div>
			</div>
