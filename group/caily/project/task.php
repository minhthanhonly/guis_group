        ...
        <!-- <div class="d-flex align-items-center mb-2">
            ...
            <div class="d-flex align-items-center flex-wrap">
                <div v-for="member in projectMembers" :key="member.user_id"
                    ...
                    <span v-else class="avatar-initial rounded-circle bg-label-primary" style="width:32px;height:32px;display:inline-flex;align-items:center;justify-content:center;">{{ getInitials(member) }}</span>
                </div>
                    ...
                    <div class="task-col-assignee">
                        <div class="d-flex align-items-center flex-wrap gap-2 py-2 pe-2 inline-assignee-picker">
                            <template v-if="getPrimaryAssigneeId(task)">
                                <div class="avatar me-1" style="cursor: pointer;" data-bs-toggle="tooltip" :title="getMemberDisplayName(projectMembers.find(m => m.user_id == getPrimaryAssigneeId(task)) || { user_name: task.assigned_to_name || getPrimaryAssigneeId(task), user_ruby: task.assigned_to_user_ruby, userid: task.assigned_to_userid })" @click="openAssigneeModal(task._inlineIndex)">
                                    <span class="avatar-initial rounded-circle bg-label-primary">{{ getInitials(projectMembers.find(m => m.user_id == getPrimaryAssigneeId(task)) || { user_name: task.assigned_to_name || '', user_ruby: task.assigned_to_user_ruby, userid: task.assigned_to_userid }) }}</span>
                                    <img v-if="shouldShowMemberAvatar(projectMembers.find(m => m.user_id == getPrimaryAssigneeId(task)) || { user_image: task.assigned_to_user_image, userid: task.assigned_to_userid, user_id: getPrimaryAssigneeId(task) })" class="rounded-circle" :src="getAvatarSrc(projectMembers.find(m => m.user_id == getPrimaryAssigneeId(task)) || { user_image: task.assigned_to_user_image })" :alt="projectMembers.find(m => m.user_id == getPrimaryAssigneeId(task))?.user_name || task.assigned_to_name" style="display:none;" @load="$event.target.style.display='block'; if ($event.target.previousElementSibling) $event.target.previousElementSibling.style.display='none';" @error="handleAvatarError(projectMembers.find(m => m.user_id == getPrimaryAssigneeId(task)) || { user_image: task.assigned_to_user_image, userid: task.assigned_to_userid, user_id: getPrimaryAssigneeId(task) })">
                                </div>
                            </template>
                            <template v-else>
                        ...
                        <div class="d-flex align-items-center flex-wrap py-2 pe-2">
                            <template v-if="getCreatorMemberForInlineTask(task)">
                                <div class="avatar me-1" data-bs-toggle="tooltip" :title="getCreatorTooltip(getCreatorMemberForInlineTask(task))">
                                    ...
                                    <img v-if="shouldShowCreatorAvatar(getCreatorMemberForInlineTask(task))" class="rounded-circle" :src="getAvatarSrc(getCreatorMemberForInlineTask(task))" :alt="getCreatorTooltip(getCreatorMemberForInlineTask(task))" style="display:none;" @load="$event.target.style.display='block'; if ($event.target.previousElementSibling) $event.target.previousElementSibling.style.display='none';" @error="handleAvatarError(getCreatorMemberForInlineTask(task))">
                                </div>
                            </template>
                        ...
                        <div class="d-flex align-items-center flex-wrap">
                            <template v-if="getPrimaryAssigneeId(task)">
                                <div class="avatar me-1 position-relative" :data-userid="projectMembers.find(m => m.user_id == getPrimaryAssigneeId(task))?.userid || task.assigned_to_userid" data-bs-toggle="tooltip" :title="getAssigneeTooltip(task, getPrimaryAssigneeId(task))">
                                    <span class="avatar-initial rounded-circle bg-label-primary" @click="removeAssignee(task, getPrimaryAssigneeId(task))">{{ getInitials(getAssigneeMember(task, getPrimaryAssigneeId(task)) || { user_name: task.assigned_to_name || '', user_ruby: task.assigned_to_user_ruby, userid: task.assigned_to_userid }) }}</span>
                                    <img v-if="shouldShowMemberAvatar(projectMembers.find(m => m.user_id == getPrimaryAssigneeId(task)) || { user_image: task.assigned_to_user_image, userid: task.assigned_to_userid, user_id: getPrimaryAssigneeId(task) })" class="rounded-circle" :src="getAvatarSrc(projectMembers.find(m => m.user_id == getPrimaryAssigneeId(task)) || { user_image: task.assigned_to_user_image })" :alt="projectMembers.find(m => m.user_id == getPrimaryAssigneeId(task))?.user_name || task.assigned_to_name" style="display:none;" @load="$event.target.style.display='block'; if ($event.target.previousElementSibling) $event.target.previousElementSibling.style.display='none';" @error="handleAvatarError(projectMembers.find(m => m.user_id == getPrimaryAssigneeId(task)) || { user_image: task.assigned_to_user_image, userid: task.assigned_to_userid, user_id: getPrimaryAssigneeId(task) })">
                        ...
                        <div class="d-flex align-items-center flex-wrap py-2 pe-2">
                            <template v-if="getCreatorMember(task)">
                                <div class="avatar me-1" :data-userid="getCreatorMember(task).userid" data-bs-toggle="tooltip" :title="getCreatorTooltip(getCreatorMember(task))">
                                    ...
                                    <img v-if="shouldShowCreatorAvatar(getCreatorMember(task))" class="rounded-circle" :src="getAvatarSrc(getCreatorMember(task))" :alt="getCreatorTooltip(getCreatorMember(task))" style="display:none;" @load="$event.target.style.display='block'; if ($event.target.previousElementSibling) $event.target.previousElementSibling.style.display='none';" @error="handleAvatarError(getCreatorMember(task))">
                                </div>
                            </template>
                            <span v-else class="text-muted small">—</span>
                        </div>
                ...
                <div class="modal-body">
                    <div class="d-flex flex-wrap">
                        <div v-for="member in projectMembers" :key="member.user_id" class="m-2 text-center" style="cursor:pointer;">
                            <div @click="toggleAssignee(member.user_id)" :class="{'border border-primary': assigneeModal.selected.includes(member.user_id)}" class="avatar avatar-md" style="display:inline-block;padding:2px;">
                                <span class="avatar-initial rounded-circle bg-label-primary">{{ getInitials(member) }}</span>
                                <img v-if="shouldShowMemberAvatar(member)" class="rounded-circle" :src="getAvatarSrc(member)" :alt="member.user_name" style="display:none;" @load="$event.target.style.display='block'; if ($event.target.previousElementSibling) $event.target.previousElementSibling.style.display='none';" @error="handleAvatarError(member)">
                            </div>
                                                    ...
                                                    <div class="d-flex flex-row align-items-start justify-content-start me-3" style="min-width:160px;">
                                                        <div class="d-flex flex-column align-items-center justify-content-start" style="width:40px;">
                                                            <div class="avatar">
                                                                ...
                                                                <img v-if="log.user_image" :src="'/assets/upload/avatar/' + log.user_image" alt="avatar" class="rounded-circle" style="display:none;" @load="$event.target.style.display='block'; if ($event.target.previousElementSibling) $event.target.previousElementSibling.style.display='none';" @error="$event.target.style.display='none'">
                                                            </div>
                                                        </div>
