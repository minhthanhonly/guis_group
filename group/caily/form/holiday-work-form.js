...
export default {
  ...
  data() {
    return {
      ...
      errors: {},
      modalTitle: this.mode === 'edit' ? '休日勤務申請編集' : '休日勤務申請書',
      submitting: false,
      approvers: [],
      originalData: null
    };
    ...
    async submit(status = 'pending') {
      ...
      try {
        ...
        if (this.mode === 'edit') {
          ...
          const payload = Object.assign({ type: 'holiday_work', status }, payloadBase);
    ...
    <div class="position-relative">
      ...
      <div class="modal-body">
        <form @submit.prevent="submit('pending')"><fieldset :disabled="mode === 'print'">
          <div v-if="mode === 'print'" class="mb-3 row">
            ...
            <div class="col-sm-9">
              <span class="request-print-text">{{ printHolidayWorkDateTime() }}</span>
            </div>
          </div>
          <template v-else>
          <div class="mb-3 row">
      ...
      <div class="text-muted small" v-if="mode==='add'">
          <ul>
            ...
            <li>申請時間と実際の出退勤打刻に30分以上の差が生じた場合は速やかに再申請をして承認を受けてください。（再申請がない場合は当初の申請内容を基準とします）  </li>
          </ul>
        </div>
    </div>
  `
