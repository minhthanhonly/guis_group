...
CREATE TABLE IF NOT EXISTS `groupware_projects` (
  ...
  `status` enum('draft','open','confirming','quotation','contract','waiting_documents','in_progress','waiting_invoice','completed','paused','cancelled','deleted') DEFAULT 'draft',
  ...
  `estimate_status` enum('未発行','見積作成中','発行済','発行済み','承認済み','却下','調整','無償') DEFAULT '未発行',
  `invoice_status` enum('未発行','請求準備','発行済','発行済み','承認済み','却下','調整','無償') DEFAULT '未発行',
  `tags` text DEFAULT NULL,
  `energy_drawing_share_status` varchar(20) DEFAULT NULL,
  `energy_drawing_share_reason` varchar(50) DEFAULT NULL,
  `energy_drawing_share_note` text DEFAULT NULL,
  `energy_drawing_share_at` datetime DEFAULT NULL,
  `energy_drawing_share_by` varchar(50) DEFAULT NULL,
  PRIMARY KEY (`id`),
